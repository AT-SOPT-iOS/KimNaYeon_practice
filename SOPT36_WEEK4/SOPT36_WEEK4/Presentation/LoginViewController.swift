...
final class LoginViewController: UIViewController {
    ...
    @objc private func registerButtonTap() {
        Task {
            do {
                let response = try await AuthService.shared.signup(with: SignupRequestDTO(loginId: self.loginId, password: self.password, nickname: self.nickName))
    ...
    @objc private func loginButtonTap() {
        Task {
            do {
                let response = try await AuthService.shared.signin(with: SigninRequestDTO(loginId: self.loginId, password: self.password))
                
                let alert = UIAlertController(
                    title: "로그인 성공",
                    message: "환영합니다, 닉네임을 찾고 싶을 땐: \(response.userId)",
                    preferredStyle: .alert
                )
